...
PARTICIPANT_NUMBER = 'poste1-mardi'
...
class Gui(tk.Tk):
    ...
    def __init__(self, loop):
        ...
        self.colorGradient[0] = "#FF0000"
        self.colorGradient[-1] = "#00FF00"
    ...
    async def test(self):
        while self.running:
            # async with BleakClient("00:A0:50:A6:43:DE") as client_hexoskin:
            async with BleakClient("20AA4559-BB76-9AC3-A4B9-6D341509860A") as client_hexoskin:
                ...
                while self.running and await client_hexoskin.is_connected():
                    ...
                    if(len(self.activity) > self.len_activity and len(self.respiration) > self.len_respiration):
                        ...
                        if(len(self.respiration) >= 30 and len(self.activity_g) >= 30):
                            # Calculate the window parameters
                            std_respiration = np.std(self.respiration[-31:])
                            std_activity = np.std(self.activity_g[-31:])
                            mean_activity = np.mean(self.activity_g[-31:])
                            ...
                            if ei > 100:
                                ...
                                color_gradient = 0
                            elif(ei>=0 and ei<=100):
                                color_gradient = ei
                                
                            self.root.configure(bg=self.colorGradient[color_gradient])
                            message = str(engagement_index) + str(self.colorGradient[color_gradient])
                            ip = "255.255.255.255"
                            # ip = "192.168.137.1"
                            port = 1234
